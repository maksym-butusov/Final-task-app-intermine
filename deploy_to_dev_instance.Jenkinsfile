...
node{
  ...
  stage('Check Availability APP page') {
   ...
   sshagent (credentials: ['stage']) {
    sh 'ssh -o StrictHostKeyChecking=no -l inception 35.228.161.55 "cd docker-intermine-gradle && docker-compose -f dockerhub.docker-compose.yml down && docker system prune -a -f"'
}
